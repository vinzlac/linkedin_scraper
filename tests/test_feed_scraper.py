...
class TestFeedScraperUnit:
    ...
    def test_extract_time_simple(self):
        ...
        assert scraper._extract_time_from_text("2h • Author") == "2h"
        assert scraper._extract_time_from_text("3 days ago") == "3 days ago"
        assert scraper._extract_time_from_text("") is None
        assert scraper._extract_time_from_text(None) is None
    ...
    @pytest.mark.asyncio
    async def test_scroll_for_more_posts_calls_end_key(self):
        scraper = self._make_scraper()
        await scraper._scroll_for_more_posts()
        scraper.page.keyboard.press.assert_called_once_with('End')
        ...
        scraper.page.evaluate = AsyncMock(return_value=[
            {
                ...
                'text': 'Contenu du post de test suffisamment long pour passer le filtre.',
                'timeText': '2h • 2nd',
                'reactions': '42',
                'comments': '7',
                'reposts': '3',
        ...
        scraper.page.evaluate = AsyncMock(return_value=[
            {
                ...
                'authorName': '',
                'authorUrl': '',
                'text': 'Post sans auteur détecté mais texte suffisant.',
                'timeText': '',
                'reactions': '',
                'comments': '',
                'reposts': '',
